...
module FSM_Light(
    ...
    parameter   S_LED_00 = 2'b00,  //  define function, 8'd1 (10진수)
                S_LED_01 = 2'b01,
                S_LED_10 = 2'b10,
                S_LED_11 = 2'b11;
    ...
    always @(curState or i_button) begin
        case (curState)
            S_LED_00 : begin
                if (i_button[0]) nextState <= S_LED_01;  //  1줄밖에 없어서 begin end 필요없음
                else if (i_button[1]) nextState <= S_LED_11;
                else          nextState <= S_LED_00;
            end
            S_LED_01 : begin
                if (i_button[0]) nextState <= S_LED_10;  //  1줄밖에 없어서 begin end 필요없음
                else if (i_button[1]) nextState <= S_LED_00;
                else          nextState <= S_LED_01;
            end
            S_LED_10 : begin
                if (i_button[0]) nextState <= S_LED_11;  //  1줄밖에 없어서 begin end 필요없음
                else if (i_button[1]) nextState <= S_LED_01;
                else          nextState <= S_LED_10;
            end
            S_LED_11 : begin
                if (i_button[0]) nextState <= S_LED_00;  //  1줄밖에 없어서 begin end 필요없음
    ...
    //상태에 따른 동작
    always @(curState) begin  //  zz high impedence, xx floating state
        case (curState)
